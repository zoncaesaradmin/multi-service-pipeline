...
if [ -d "../../client/golang/src/platformkit" ]; then
    (
        cd ../../client/golang/src/platformkit && make clean 2>/dev/null || true
    )
fi
...
printf '%s\n' "src/service" "src/testrunner" "../../client/golang/src/platformkit" | while read -r path; do
    [ -e "$path" ] && echo "$path"
done
