...
```bash
# From the root directory (kritikos/)
make test
...
- **Location**: `/tmp/kritikos-messagebus/`
...
ls -la /tmp/kritikos-messagebus/

# View input messages
cat /tmp/kritikos-messagebus/test_input/0000000000.json

# View output responses
cat /tmp/kritikos-messagebus/test_output/0000000000.json | jq .
```

### Message Bus Cleanup
```bash
# Clean message bus for fresh test
rm -rf /tmp/kritikos-messagebus
```
...
4. Check port conflicts (service uses localhost:4577)

### Message Bus Issues
1. Check message bus directory: `ls -la /tmp/kritikos-messagebus/`
2. Verify message format: `cat /tmp/kritikos-messagebus/test_*/000*.json | jq .`
3. Clean and retry: `rm -rf /tmp/kritikos-messagebus && make test`
...
4. **Message Bus Debugging**: Check /tmp/kritikos-messagebus/ for message flow
...
### Local vs Production
- **Local**: File-based message bus in `/tmp/kritikos-messagebus/`
