...
cleanup() {
    ...
    if [ -d "/tmp/kritikos-messagebus-test" ]; then
        rm -rf "/tmp/kritikos-messagebus-test"
        log_info "Cleaned up LocalBus message data"
    fi
    
    if [ -d "/tmp/kritikos-messagebus-offsets" ]; then
        rm -rf "/tmp/kritikos-messagebus-offsets"
...
main() {
    ...
    LOCAL_BUS_BASE="/tmp/kritikos-messagebus-test"
    LOCAL_BUS_OFFSETS="/tmp/kritikos-messagebus-offsets"
